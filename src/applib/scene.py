...
from applib.overlay import annotate_frame
...
class SceneDescriber:
    ...
    def describe(self, frame_rgb, tracks: list[Track], question: str = "") -> str:
        """Annotate the frame, ask SmolVLM about it, and return what to say out loud.

        `question` is asked verbatim when a caller has one - a C2D `scene` command may carry
        "what is the person holding". It is passed through untouched: the model was trained on
        plain questions, and rewriting one here would only hide what was actually asked.
        """
        with self._lock:
            ...
            description = "".join(self._model.process_message(asked)).strip()
            print(f"[vlm] {perf_counter() - started:.1f}s | {len(tracks)} boxes | "
                  f"{asked!r} -> {description!r}")

        if question.strip():
            return description
        # For the default question the count is spoken by us and the description by the model:
        # hearing both makes it obvious when the model is describing something we never detected.
