#!/bin/bash
#
# Prepare a board to run hyena. Replaces NXP's install.sh, which does considerably more than we
...
if [ ! -d nxp-lib/src/vit ]; then
    ...
    echo "       (hyena still runs the video loop without it: ./run.sh --no-voice)" >&2
    exit 1
fi
...
# point hyena at /opt/dm-eiq instead. vocabulary.py also shells out to it for phonemes.
