#!/usr/bin/env python3
"""hyena pilot - the anti-theft demo, driven by voice and by /IOTCONNECT, on the FRDM-IMX95.

gazelle put elephant's eyes (camera -> YOLO11n on Neutron -> tracking -> faces -> alarm) and
falcon's ears (NXP's eIQ wake word, STT, TTS and SmolVLM) in one process, taking spoken commands.
hyena connects that to the cloud: the same commands arrive as /IOTCONNECT C2D messages, the alarm
state and what is on screen go out as telemetry, and a snapshot lands in S3.

    ./run.sh                                   # NPU for YOLO and SFace, voice on, cloud on
    ./run.sh --no-iotc                         # no cloud (also the automatic answer if unconfigured)
...
The voice thread and the /IOTCONNECT thread both do their own loading and connecting, so the picture
is live in about a second. Watch the HUD's `voice:` and `cloud:` lines for when each is ready.
"""
...
VERSION = "hyena-1.0"        # reported as the 'version' telemetry attribute
...
def main() -> None:
    ...
    cloud = build_iotc_client(args, service, telemetry, overlay)
...
def build_iotc_client(args, service: CommandService, telemetry: TelemetryState,
                      overlay: Overlay) -> IotcClient | None:
    ...
    refuses to start over a missing certificate is worse than a demo with no dashboard.
    """
    ...
    if not iotc.is_configured(config_path):
        reason = "SDK not installed" if not iotc.IS_SDK_AVAILABLE else f"no {config_path} + certs"
    ...
    client = IotcClient(
        ...
        on_status=overlay.set_cloud_status, interval_s=args.iotc_interval,
        is_verbose=args.iotc_verbose,
    )
