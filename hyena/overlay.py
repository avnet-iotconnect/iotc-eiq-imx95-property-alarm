...
def annotate_frame(frame_rgb, tracks: list[Track], hud_lines=()):
    ...
    Two callers, one reason. The Cairo path above paints on the preview, which lives inside
    GStreamer and **cannot be read back** - there is no way to ask the compositor for those pixels
    (see README, "taking a screenshot"). So anything that needs the picture *outside* the screen
    re-renders it here:

    - `scene.py` sends it to the VLM. `USER_GOAL.md` asks for "describe the thief highlighted in
      the orange rectangle", and asking about boxes the model cannot see is how you get a confident
      answer about the wrong thing.
    - the `snapshot` command writes it to disk, which is also what /IOTCONNECT's keyframe upload
      will want later.
...
def save_snapshot(path, frame_rgb, tracks: list[Track], hud_lines=()) -> str:
    """Write what the screen is showing to a JPEG. Returns the path, for the spoken confirmation."""
    import cv2
