...
In the dolphin pilot this logic lived inline in the frame loop. Here it's wrapped in a `Detector`
class so `main.py` can build it once and just call `detector.detect(frame)` - the same shape as
