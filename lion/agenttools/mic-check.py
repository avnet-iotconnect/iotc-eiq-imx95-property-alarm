...
demo permanently — it is the first thing to run when a booth microphone misbehaves.
    ...
    agenttools/mic-check.py --list                  # what audio hardware this board has
    agenttools/mic-check.py                         # record, meter and play back, using audio.json
    agenttools/mic-check.py --loop                  # keep going: record, listen, adjust, repeat
    agenttools/mic-check.py --mic Fifine --range 8  # try another mic and another hardware gain
    agenttools/mic-check.py --gain 4                # digital boost on playback only

Devices come from config/audio.json and may be named by alias, by any substring of a card name, or
by a full ALSA device string — see applib/audio_devices.py. Anything on the command line wins.
...
from applib import audio_devices  # noqa: E402
