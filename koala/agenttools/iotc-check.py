...
the SDK, the device config and the certificate pair sitting in the pilot root above this directory.
Run it once after deploying to a new board, and again whenever the demo's HUD says `cloud: offline`
and you want to know whether the problem is the board, the account or the device template.

    agenttools/iotc-check.py                 # connect, report S3/KVS, send one telemetry message
    agenttools/iotc-check.py --listen 60     # ... then print C2D commands for a minute (acked as failed)
    agenttools/iotc-check.py --upload        # ... and upload capture.jpg, proving the S3 path
    agenttools/iotc-check.py --webrtc        # ... and open the signalling channel, proving WebRTC
