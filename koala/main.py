#!/usr/bin/env python3
"""koala - the anti-theft demo on the FRDM-IMX95, now with an LLM on the Ara-240.

iguana had the whole demo bar one thing: everything a visitor could ask for had to be a *known*
command, spoken or pressed. koala adds `ask` - a plain-English sentence from the /IOTCONNECT
dashboard, answered by Qwen2.5-7B running on the Ara-240 DNPU, whose tools are the demo's own
...
The layout, which is the other thing koala changes (see work/GUIDELINES.md):
...
from applib import audio_devices, iotc, vocabulary, webrtc  # noqa: E402
...
VERSION = "koala-1.0"        # reported as the 'version' telemetry attribute
...
def parse_args() -> argparse.Namespace:
    ...
    llm = parser.add_argument_group("the LLM on the Ara-240 (new in koala)")
