#!/bin/bash
#
# Prepare a board to run koala. Replaces NXP's install.sh, which does considerably more than we
...
#   3. create a --system-site-packages venv, so the BSP's numpy, onnxruntime, tflite_runtime, cv2
#      and gi stay visible alongside what pip adds
#   4. pip install requirements.txt
#
# The vision models (YOLO + face) are NOT fetched here: converting them needs the neutron-converter,
# which does not run on the board. Build them on the host and copy models/ over with the rest of
# this directory -- see README.md.
#
# The LLM is not installed here either. `connector/` is NXP's eIQ AAF Connector with its own venv,
# its own several-gigabyte install and its own model load, and it stays a separate invocation:
#
#     cd connector && ./install.sh && ./run.sh
#
# Everything it writes lives in nxp-lib/, /opt/dm-eiq and the venv. Nothing lands in /usr, so
# `rm -rf` of those paths returns the image to its shipped state.
#
...
for argument in "$@"; do
    case "$argument" in
        ...
        *) echo "Unknown option: $argument"; sed -n '3,27p' "$script" | sed 's/^# \?//'; exit 1 ;;
    esac
done

# --- NXP's payload ---------------------------------------------------------------------------
# 139 MB of compiled modules and encrypted weights, hosted rather than committed: the licence does
...
if [ "$skip_payload" = false ] && [ ! -d nxp-lib/src/vit ]; then
    echo "=== downloading the eIQ payload (139 MB) ==="
...
if [ ! -d nxp-lib/src/vit ]; then
    ...
    echo "       (koala still runs the video loop without it: ./run.sh --no-voice)" >&2
    exit 1
fi
...
# point koala at /opt/dm-eiq instead. vocabulary.py also shells out to it for phonemes.
...
if [ "$skip_pip" = false ]; then
    ...
    ./venv/bin/python -c "
import cv2, numpy, onnxruntime, tflite_runtime
print('numpy       ', numpy.__version__, numpy.__file__)
print('cv2         ', cv2.__version__)
print('onnxruntime ', onnxruntime.__version__)
assert 'NeutronExecutionProvider' in onnxruntime.get_available_providers(), 'Neutron provider lost!'
assert hasattr(cv2, 'FaceDetectorYN'), 'cv2 has no face module -- face recognition will not work'
print('Neutron provider present, cv2 face classes present')
"
fi

echo
echo "Done. Next:"
echo "  ./run.sh --prefetch          # once, on a good network: pulls SmolVLM from Hugging Face"
echo "  ./run.sh                     # the demo -- say \"Hey NXP\", then a command"
echo
echo "The LLM behind the 'ask' command is installed separately, and can be left for later:"
echo "  cd connector && ./install.sh # once: ~1.2 GB, several minutes"
echo "  cd connector && ./run.sh     # ~4 minutes to load the 7B onto the Ara-240"
