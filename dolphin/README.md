# dolphin - YOLOv8 on FRDM-IMX95, CPU vs Neutron NPU
...
- `yolo.py` - the math: letterbox, quantize, decode YOLOv8 output, NMS, box mapping.
...
YOLOv8 takes one square RGB image and predicts, at 2100 anchor points, a box and 80 class scores.
...
**Why the model output looks "raw".** YOLOv8's box decode (DFL softmax + anchor math) does not
...
standard: the NPU runs the heavy convolutions (one big `neutronOp`), the CPU does the cheap decode.
See `work/export-neutron.py` for the matching export patch.

## Run (on the board, from ~/dolphin)
```
./run.sh --model yolov8n_int8.tflite                # CPU
./run.sh --model yolov8n_neutron.tflite --delegate  # Neutron NPU
```
