...
This is deliberately the seam for the next subproject: a face model can attach an identity to a track
(fill Track.identity, e.g. a name) and the overlay would show that instead of the bare number.
"""
...
@dataclass
class Track:
    """One tracked object. `short_id` is what the screen shows; `identity` is the future face hook."""
    ...
    identity: str | None = None  # reserved: a face model fills this later (e.g. a person's name)
