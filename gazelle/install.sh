#!/bin/bash
#
# Prepare a board to run gazelle. Replaces NXP's install.sh, which does considerably more than we
...
#
# The vision models (YOLO + face) are NOT fetched here: they are built on the host and pushed by
# deploy.sh, because converting them needs the neutron-converter, which does not run on the board.
#
...
for argument in "$@"; do
    case "$argument" in
        ...
        *) echo "Unknown option: $argument"; sed -n '3,22p' "$script" | sed 's/^# \?//'; exit 1 ;;
    esac
done
...
if [ ! -d nxp-lib/src/vit ]; then
    ...
    echo "       (gazelle still runs the video loop without it: ./run.sh --no-voice)" >&2
    exit 1
fi
...
# point gazelle at /opt/dm-eiq instead. vocabulary.py also shells out to it for phonemes.
