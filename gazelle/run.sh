#!/usr/bin/env bash
# run.sh - launch the gazelle pilot ON THE BOARD.
#   ./run.sh                              # NPU (default): YOLO + SFace on Neutron, voice on
#   ./run.sh --no-voice                   # video only; needs no eIQ payload and no venv packages
...
# Uses the pilot venv at ~/gazelle/venv (created by install.sh).
